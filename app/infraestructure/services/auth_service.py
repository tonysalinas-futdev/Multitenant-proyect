...
class AuthService():
    ...
    def create_acces_token(self, data:dict, expires_time:Optional[timedelta]=None ):
        ...
        access_token=jwt.encode(data,self.jwt_secret_key,self.algorithm)
        return access_token

    def create_refresh_token(self,user_id:int,expires_time:Optional[timedelta]=None):
        if expires_time:
            token_duration=datetime.now(timezone.utc)+expires_time
        else:
    ...
    async def verify_refresh_token(self, token_value:str):
        hash_token_value=hashlib.sha256(token_value)
    ...
    async def create_access_and_refresh_tokens(self,user_data:Users):
        ...
        access_token=self.create_acces_token(access_token_data)
